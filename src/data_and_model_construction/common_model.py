...
def create_model(data_dict):
    ...
    produccion_vars = mdl.continuous_var_dict(prod_names, name=prod_names)#, key_format="%s")

    # Debug
    # print(f"products: {products}")
    # print(f"prod_names: {prod_names}")
    # print(f"produccion_vars: {produccion_vars}")
    # print(f"resources: {resources}")
    # print(f"consumptions: {consumptions}")
    # print(f"")
    # print(f"produccion_vars['A']: {produccion_vars['A']}")
    # --- constraints ---

    # resources disp equipo and consumptions
    mdl.add_constraints((mdl.sum(produccion_vars[p[0]] * consumptions[res[0]][products.index(p)] for p in products) <= res[1], 'Disp_%s' % res[0]) for res in resources)

    # max demand
    #mdl.add_constraints((produccion_vars[p] <= p[2], 'DemandMax_%s' % p[0]) for p in products)
    mdl.add_constraints((produccion_vars[p[0]] <= p[2], 'DemandMax_%s' % p[0]) for p in products)

    # min demand
    mdl.add_constraints((produccion_vars[p[0]] >= p[3], 'DemandMin_%s' % p[0]) for p in products)

    # --- print information ---
    mdl.print_information()

    total_benefit = mdl.sum(produccion_vars[p[0]] * p[1] for p in products)
