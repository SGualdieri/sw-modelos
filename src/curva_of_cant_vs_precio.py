...
class CurvaDeOferta(PlotKind):
    ...
    ##########################################################################################################
    ##########################################################################################################
    ########### AUX: (all or most of) the following methods should be in a price_iterator ####################
    ##########################################################################################################

    def get_prod_var_for(self, product_name, produccion_vars):
    ...
    def perform_sensitivity_analysis(self, mdl, prod_name, produccion_vars):
        ...
        prod_var=self.get_prod_var_for(prod_name, produccion_vars)
    ...
    def iterate_over_price(self, prod_name, prod_var, mdl, products, produccion_vars): # aux: var mdl, 'm', y funciones.
        ...
        current_quantity_value = self.get_y(prod_var)

        return iterate_internal(prod_name, prod_var, current_price_value, current_quantity_value, mdl, products, produccion_vars, self.get_y, self.perform_sensitivity_analysis, self.solve_model_for_price) # aux: var mdl, 'm', y funciones.
    ...
    def iterate_over_price_for_var(self, product_name, mdl, products, produccion_vars):
        ...
        # Iteramos
        current_price_value, prices, quantities = self.iterate_over_price(product_name, prod_var, mdl, products, produccion_vars)
        
        # Le agregamos el punto de x=0 al inicio, porque la función que itera solo contempla números no negativos    
        price = 0     
        _ = self.solve_model_for_price(product_name, price, mdl, products, produccion_vars)
